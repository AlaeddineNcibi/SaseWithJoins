...
final class BTreeNode<K, V> implements Serializer<BTreeNode<K, V>> {
	...
	/**
	 * Children nodes (recids) associated with keys. (Only valid if non-leaf
	 * node)
	 */
	...
	BTreeNode(BTree<K, V> btree, K key, V value) throws IOException {
		...
		_values[BTree.DEFAULT_SIZE - 1] = null; // I am the root BTreeNode for
												// now
	 ...
	 *
	 * @param height
	 *            Height of the current BTreeNode (zero is leaf node)
	 * @param key
	 *            The key
	 * @return TupleBrowser positioned just before the given key, or before next
	...
	// Get the Node that contains the lowest range value

	BTreeNode<K, V> findRange(int height, K k1) throws IOException {
		byte index = findChildren(k1, false); // /add the inclusive later
		height -= 1;
		if (height == 0) {
			/**
			 ...
			 */
			if (_keys[index] == null) // || compare(k1, key2) != 0)
				return null;
			// leaf node
			return this;
		} else {
			...
			BTreeNode<K, V> child = loadNode(_children[index]);
			// child.dump(height);
			return child.findRange(height, k1);
		}

	}

	public void dosomethingWithRange(BTreeNode<K, V> rootNode, int height, K k1) throws IOException {

		BTreeNode<K, V> start = rootNode.findRange(height, k1);

		while (start._next != 0) {
			// start.dump(height);
			start = loadNode(start._next);
			start.dump(height);
		}
	...
	V findValue(int height, K key) throws IOException {
		...
		if (height == 0) {
			...
			K key2 = _keys[index];
			// // get returns the matching key or the next ordered key, so we
			// must
			// // check if we have an exact match
	...
	void delete() throws IOException {
		if (_isLeaf) {
			if (_next != 0) {
				...
				if (nextNode._previous == _recid) { // this consistency check
													// can be removed in
													// production code
			...
			if (_previous != 0) {
				BTreeNode<K, V> previousNode = loadNode(_previous);
				if (previousNode._next != _recid) { // this consistency check
													// can be removed in
													// production code
	...
	InsertResult<K, V> insert(int height, K key, final V value, final boolean replace) throws IOException {
		...
		if (height == 0) {
			...
			// reuse InsertResult instance to avoid GC trashing on massive
			// inserts
			...
			if (compare(_keys[index], key) == 0) {
				...
				if (isLazyRecord) {
					...
					/**
					 * Update the number of runs and windows for same keys
					 */
					updateValue(value, (V) _values[index]);
				...
				if (replace) {
					...
					// ** Add the updated classes like updating the number of
					// runs

					/**
					 * HARDCODED for NOW
					 */
					// Value v = (Value) _values[index];

					// v.set_numOfRuns(v.get_numOfRuns()
					// + ((Value) value).get_numOfRuns());
			...
			// there was an overflow, we need to insert the overflow node on
			// this BTreeNode
			if (DEBUG) {
		...
		if (index < half) {
			...
			// move lower-half of entries to overflow node, new entry stays on
			// this node
			if (DEBUG) {
				System.out.println(
	...
	// ::TODO add the event to existing list
	private void updateValue(V newVal, V oldVal) {

		/**
		 * Updating the number of runs
		 */
		int newEventCount = ((Value) oldVal).get_numOfEvents() + ((Value) newVal).get_numOfEvents();
		((Value) oldVal).set_numOfEvents(newEventCount);

		/**
		 * Updating the windows
		 */

		((Value) oldVal).get_window().add(((Value) newVal).getTimestamp());
	 ...
	 */
	public byte findChildren(final K key, final boolean inclusive) {
		int left = _first;
		int right = BTree.DEFAULT_SIZE - 1;
		int middle;
		final int D = inclusive ? 0 : 1;

		// binary search, check if its correct, find the exact value or the
	 ...
	 */
	public BTreeNode<K, V> loadNode(long recid) throws IOException {
	...
	public void dump(int height) {
		...
		for (int i = 0; i < BTree.DEFAULT_SIZE; i++) {
			if (_isLeaf) {
				...
				Value v = ((Value) _values[i]);
				if (v == null)
					continue;
				// StockEvent se = (StockEvent) v.event;
				// System.out.println("Price " + se.price);
				// System.out.println("Volume " + se.vol);
	...
	public void serialize(DataOutput oos, BTreeNode<K, V> obj) throws IOException {
		...
		// note: It is assumed that BTreeNode instance doing the serialization
		// is the parent
	...
	private void writeKeys(DataOutput oos, K[] keys, final int firstUse) throws IOException {
		...
		if ((_btree._comparator == Utils.COMPARABLE_COMPARATOR || _btree._comparator == null)
			...
			for (int i = firstUse; i < BTree.DEFAULT_SIZE; i++) {
				...
				// special case to exclude Long.MIN_VALUE from conversion,
				// causes problems to LongPacker
	...
	static final class Browser<K, V> implements BTree.BTreeTupleBrowser<K, V> {
		...
		public boolean getNext(BTree.BTreeTuple<K, V> tuple) throws IOException {
			...
			if (_node == null) {
				// last record in iterator was deleted, so iterator is at end of
				// node
				return false;
			}
		...
		public boolean getPrevious(BTree.BTreeTuple<K, V> tuple) throws IOException {
			...
			if (_node == null) {
				// deleted last record, but this situation is only supportedd on
				// getNext
				throw new InternalError();
			}
		...
		public void remove(K key) throws IOException {
			...
			// browser is positioned just before value which was currently
			// deleted, so find if we have new value
	...
	/**
	 * This method is used for delta compression for keys. Writes the contents
	 * of buf to the DataOutput out, with special encoding if there are common
