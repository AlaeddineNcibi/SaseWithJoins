...
public class SpaceFillingCurves {
	 ...
	 * non-instantiability.
	 */
	public SpaceFillingCurves() {
	}

	/**
	 * Computes the hilbert value for two given integers. The number of bits of
	 * x and y to be considered can be determined by setting the parameter mask
	 * appropriately.
	 * 
	 * @param x
	 ...
	 * @param mask
	 *            the bitmask containing exactly the highest bit to be
	 *            considered. If all bits of x and y should be taken into
	 *            account, choose the value 1<<31.
	...
	/**
	 * Computes for a given hilbert value the origin values for two dimensions
	 * resulting in that hilbert value. The number of bits of the resulting
	 * values to be considered can be determined by setting the parameter mask
	 * appropriately.
	 * 
	 * @param hilbert
	 *            the given hilbert value
	 * @param mask
	 *            the bitmask containing exactly the highest bit of the
	 *            resulting two values to be considered. If all bits should be
	 *            taken into account, choose a value of 1<<31.
	 * @return an array containing the two values whose hilbert value is equal
	...
	/**
	 * Computes for a given hilbert value the origin values for two dimensions
	 * resulting in that hilbert value.
	 * 
	 * @param hilbert
	 *            the given hilbert value
	 * @return an array containing the two values whose hilbert value is equal
	...
	/**
	 * Computes the peano value for two given integers. The number of bits of x
	 * and y to be considered can be determined by setting the parameter mask
	 * appropriately.
	 * 
	 * @param x
	 *            the value of the first dimension
	...
	/**
	 * Computes for a given peano value the values for two dimensions resulting
	 * in that peano value. The number of bits of the resulting values to be
	 * considered can be determined by setting the parameter mask appropriately.
	 * 
	 * @param peano
	 *            the given peano value
	 * @param mask
	 *            the bitmask containing exactly the highest bit to be
	 *            considered for the resulting two values. If all bits should be
	 *            taken into account, choose the value 1<<31.
	 * @return an array containing the two values whose peano value is equal to
	 *         the specified one
	 */
	...
	/**
	 * Computes for a given peano value the origin values for two dimensions
	 * resulting in that peano value.
	 * 
	 * @param peano
	 *            the given peano value
	 * @return an array containing the two values whose peano value is equal to
	 ...
	 * the the bit-field-longs and returns it as a BitSet. <br>
	 ...
	 *
	 * @param bitField
	 *            a long array the z-code should be computed of.
	 * @param componentPrecision
	 *            the bitField's component precision.
	...
	public static BitSet zCode(final long[] bitField, int componentPrecision) {
		...
		/*
		 * BitSet zCode = new BitSet(componentPrecision*bitField.length); long
		 * mask = 0x1L << 62; //first bit true, remainin bits: false int
		 * bitIndex = 0; //bit-index in z-code for(int i=0;
		 * i<componentPrecision; i++){ for(int d=0; d<bitField.length; d++){
		 * if((mask & bitField[d]) == mask) zCode.set(bitIndex); bitIndex++; }
		 * mask >>>= 1; } return zCode;
		 */
	}
	 ...
	 *            a long array the z-code should be computed of.
	 * @param precision
	 *            the returned BitSet's full precision.
	 * @return a BitSet representing the z-code of the given <tt>bitField</tt>.
	 * @see xxl.core.util.BitSet#BitSet(int)
	 * @see #zCode(long[], int, int)
	 */
	public static BitSet zCode2(final long[] bitField, int precision) {
		return zCode(bitField, precision / bitField.length, precision
				% bitField.length);
	}
	 ...
	 * </pre>
	 * 
	 * So the basic used precision is the array's component precision, but it
	 * can be increased by adding <tt>additionalBits</tt>.
	 *
	 * @param bitField
	 *            a long array the z-code should be computed of.
	 * @param componentPrecision
	 ...
	 *            added bits, that should be taken into consideration too, i.e.
	 *            the precision of the BitSet is incremented by this value.
	 * @return a BitSet representing the z-code of the given <tt>bitField</tt>.
	 * @see xxl.core.util.BitSet#BitSet(int)
	 */
	public static BitSet zCode(final long[] bitField, int componentPrecision,
	 ...
	 * 
	 * @param length
	 *            dimension*number of bits pro dim
	 * @param cycle
	 * @param dimension
	 * @return
	 */
	public static int[] createCyclicFunction(int length, int cycle,
			int dimension) {
	 ...
	 * 
	 * @see H. Tropf, H. Herzog: Multidimensional Range Search in Dynamically
	 *      Balanced Trees, Angewandte Informatik, 2/1981, pp 71-77
	 *
	 * @param zcode
	 * @param bitsProDim
	 *            number of bits which is needed to represent key
	 * @param dimension
	 * @return next value of z key within the query box
	 */
	public static long nextInBoxZValue(long zcode, int[] lPoint, int[] rPoint,
			int bitsProDim, int dimension) {
	 ...
	 * @return
	 */
	private static int readDimension(long zcode, int dim, int dimensions,
			int bitsProdim) {
	 ...
	 * @param dimensions
	 */
	private static long writeDimension(long zcode, int value, int dim,
			int bitsProDim, int dimensions) {
	...
	/**
	 * Computes the list of ranges which runs through the query box for a
	 * Z-Curve. This is a recursive algorithms, which cuts the box along the
	 * separation dimension (first bit which is differs) until the whole box is
	 * within the region (equal prefix:11111...111, equal prefix:000000... 0000)
	 * 
	 * @param lPoint
	 * @param rPoint
	 * @param bitsProDim
	 ...
	 * @return
	 */
	public static List<long[]> computeZBoxRanges(int[] lPoint, int[] rPoint,
			int bitsProDim, int dimension) {
		...
		int prefix = 0;
		for (suffix = 64; ((mask & zvalueLeft) == (mask & zvalueRight))
				& suffix >= 0; suffix--, mask = mask >>> 1, prefix++) {
		...
		int[] newLow = cut(zvalueLeft, zvalueRight, bitsProDim, dimension,
				prefix, true);
		int[] newHigh = cut(zvalueLeft, zvalueRight, bitsProDim, dimension,
				prefix, false);
	 ...
	 * @return
	 */
	private static int[] cut(long zvalueLeft, long zvalueRight, int bitsProDim,
			int dimension, int prefix, boolean low) {
		...
		position = position / dimension; // position in dimension from left to
											// right
		...
		if (low) { // compute left lower point of new box
			...
			dimvalue = (dimvalue >> (bitsProDim - position - 1)) << (bitsProDim
					- position - 1);

			dimvalue |= mask; // write value
			output = writeDimension(output, dimvalue, dim, bitsProDim,
					dimension);
	...
	public static void main(String[] args) {
		...
		int bitsProdim = 3;
		int dimension = 2;
		List<long[]> result = computeZBoxRanges(left, right, bitsProdim,
				dimension);
		for (long[] l : result) {
